...
class Member extends BasicAdmin {
    ...
    private $dataform = 'r_member';

    public function index() {
        $this->title = '用户列表';
        list($get, $db) = [$this->request->get(), Db::name($this->dataform)->where('status',1)];
        (isset($get['keywords']) && $get['keywords'] !== '') && $db->whereLike('users', "%{$get['keywords']}%");
    ...
    protected function _form_result($result) {
        if ($result !== false) {
            list($base, $spm, $url) = [url('@admin'), $this->request->get('spm'), url('tuanj/member/index')];
